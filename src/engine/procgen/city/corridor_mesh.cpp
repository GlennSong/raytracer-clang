...
std::vector<UnionSpine> corridorDeckSpines(
    ...
    // buildCorridorMesh uses) so `halfWidthAt` flares one lane at each gore.
...
CorridorAuthoring corridorAuthor(const CorridorDef& cIn,
                                 ...
                                 Real step) {
    // The geometry-free half of buildCorridorMesh (one-mesher P8b). Every line
    // here mirrors that function's authoring so the outputs match byte for byte;
    // the deck/ramp/pier/marking emission it interleaved is simply absent. The
    // gore band's centreline re-samples the alignment (it never read the deck's
    // rib array), which is what makes the split possible at all.
...
CorridorMeshOut buildCorridorMesh(
    ...
    for (const ExitDef& e : c.exits) {
        ...
        // SIGNAGE: overhead sign gantries are emitted once, after this loop, by
        // the shared corridorFurniture(c) — so the ONE welder's deck wears the
        // same signage. (Re-homed here, one-mesher P7.)
    ...
    // Overhead sign gantries — emitted by the shared pass so the ONE welder's
    // deck can wear the same signage (one-mesher P7).
    MeshBuilder::append(out.barrier, corridorFurniture(c));
