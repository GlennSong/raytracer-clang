# lanes — the lane-atomic road builder (ADR-0083, ADR-0085, ADR-0089)

**What it is.** One of the roads module's two builders, beside the swept lattice rather than
replacing it. It was `procgen/lanelab` and its own static library until 2026-09-20; ADR-0089
folded it in here, inside `engine_core`, compiled when `RT_ROADS_LANES` is on (the default —
it needs the Clipper2 and CDT submodules), with `lanes_tool` and `lanes_tests` as its headless
hosts. A level still picks per entity — `shape:"road"` for the lattice, `shape:"lanelab"` for
this one — until the builder key (`"builder": "lanes"`) takes over in phase 3. It
...
```
cmake --build build-viewer --target lanes_tool lanes_tests
./build-viewer/lanes_tests                                              # 9 cases: geometry seam + the four scenes' invariants
./build-viewer/lanes_tool build assets/lanelab/ring_city.json --out out/lanelab/ring_city     # summary, PASS/FAIL invariants, .glb/.svg/stats.json
./build-viewer/lanes_tool from-level assets/levels/metro_v2_test.json --out out/lanelab/metro --diamonds 6
./build-viewer/lanes_tool build out/lanelab/metro/<name>_lanelab.json --out out/lanelab/metro
```
...
**Invariants** (`lanes_tool build` prints them; `lanes_tests` asserts them): decks welded (no
...
**Is it drivable? Yes, behind an option.** `-DRT_ROADS_LANES=ON` links the lab into `engine_core`
and enables `shape:"lanelab"` in the level loader (`loadLanesEntity`, next to `loadRoadEntity`):
...
sets it on landing streets (lots stop at the outer frontage road); ring city's arterials carry it by hand. Diagnostics: `lanes_tool twin
...
lots crossed streets. He was right about the mechanism being in the code, not the data. `lanes_tool
...
`lanes_tool twin <graph> --margin <sidewalk>` now reports BROKEN BLOCKS: every block foot — the face
...
plan alone (layer styles inherited from the `<g>` groups, legend dropped). Spurs: `lanes_tool twin`
...
- **The audit runs the real pass headless.** `lanes_tool blocks <graph> [--citysim level.json] --out dir`
...
6. **Squared-off land that became no block** — the parceller's miter inset rejected whole holes over one
   ...
   `lanes_tool blocks` reports any hole that yields no block, with the reason.
...
- `driving surface has no steps (> kerb, < bridge_h)` — `surfaceSteps()` walks every deck boundary
  ...
  five lab scenes: 0. `lanes_tool build … --probe x y r`
...
- `pads:` in the block audit (`lanes_tool blocks`, exit 2; the block-pass test) — building pads and
  ...
  terraces with `lanesTerraces`: footprints inset by the feather (2 m) and clipped to the block inset by
...
`cells`, `report`. The loader (`loadLanesEntity`) obtains the level's bundle once — from disk when the key
...
- Key = graph JSON bytes + terrain grid file bytes + `citysim.renderCell` + `sizeof(Real)` + format +
  `kLanesBuildTag` (`city_producer.cpp`). **Bump the tag whenever the lab's output changes.**
...
- The invariant sweep is not part of a bake (it costs minutes on metro): `rt_bake --check` or
  `lanes_tool build` when you want it.
...
Pavement-hole fixes that came out of the prototype (`block_audit.cpp`; `kLanesBuildTag` and `kLotsBuildTag`
...
`lanes_tool build <graph> --quick --holes` lists every hole under 2000 m², what covers it, and the edge it
lies against: metro is 25 holes, 25 covered, 0 bare. `kLanesBuildTag` -> 2026-09-07.3.
...
block: the city bundle is obtained early (the same one `loadLanesEntity` reuses), its `city/e0/ground`
...
roughness and never asked for a surface. `surfaceForLanesMaterial` in `level_loader.cpp` maps the bundle's
...
metres per role and how much of each carries a barrier today, printed by `lanes_tool build … --walls`:
...
`lanes_tool build <graph> --quick --walls` prints them with the ends that stand on same-level pavement:
metro has 171 runs, 8717 m, 217 such ends. `kLanesBuildTag` -> 2026-09-07.4.
...
`kLanesBuildTag` -> 2026-09-08.2. Note the terrain a lanelab level renders is NOT the CDLOD terrain: the
