...
const char* const kLanelabBuildTag = "2026-09-08.16";
...
public:
    ...
    bundle::ProducerIdentity identity(const bundle::LevelInputs& in) const override {
        bundle::ProducerIdentity id; id.tag = kLanelabBuildTag;
        uint64_t k = bundle::fnv1aStr(std::string(kCityProducerName) + "|" + std::to_string(kCityFormatVersion) + "|" + kLanelabBuildTag + "|real" + std::to_string(sizeof(Real)));
...
void writeCityProducts(bundle::BundleWriter& w, int ordinal, const CityProducts& p) {
    ...
    w.addJson(pre + "meta", nlohmann::json{{"format", kCityFormatVersion}, {"tag", kLanelabBuildTag}, {"hasTerrain", p.hasTerrain}, {"cell", p.cell}});
