...
std::vector<TerrainFlatten> clipPadsToBlocks(const std::vector<LotBuilding>& lots, const std::vector<Poly2>& blocks, double sidewalk) {
    std::vector<TerrainFlatten> out; const double falloff = lanelabPadFalloff(sidewalk); const PolySet inside = blocksInset(blocks, falloff);
...
std::vector<TerrainFlatten> lanelabTerraces(const std::vector<TerrainFlatten>& grades, const std::vector<Poly2>& blocks, double sidewalk) {
    std::vector<TerrainFlatten> out; const double falloff = lanelabPadFalloff(sidewalk); const PolySet inside = blocksInset(blocks, falloff);
...
BlockAudit auditBlocks(const Result& r, const nlohmann::json& citysim, bool fromScene) {
    ...
    if (fromScene) {
        ...
        {   // pads and terraces against the pavement: as the loader built them before, and as it builds them now
            ...
            for (TerrainFlatten& f : lanelabTerraces(a.grown.gradeFlatten, blocks, sidewalk)) now.push_back(std::move(f));
            ...
            if (std::getenv("LANELAB_TRACE_PADS")) for (const PadConflict& pc : a.padConflicts) {
                const TerrainFlatten& f = now[static_cast<size_t>(pc.pad)]; const bool terrace = pc.pad >= static_cast<int>(now.size() - lanelabTerraces(a.grown.gradeFlatten, blocks, sidewalk).size());
