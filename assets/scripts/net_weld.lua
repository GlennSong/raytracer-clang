...
-- Lane paint is a TEXTURE, not geometry: the RoadMarkings surface reads the welded deck's
-- road-local UV (u = lateral, v = arc-length) and paints a double-yellow centre, dashed white
-- lane dividers (multilane) and solid white edges — conforming to terrain, crisp at any range.
...
m:add_solid(city.solid{ spines = hub, y = 0.4, thickness = 0.8, color = DARK, corner_radius = 5 }, PAINT)
...
m:add_solid(city.solid{ spines = g, y = 0.4, thickness = 0.8, color = DARK, corner_radius = 4 }, PAINT)
return m
