// lanelab_tool — the headless host for the lanelab road generator (ADR-0083).
//   lanelab_tool build <graph.json> [--out <dir>]     build, print the summary and invariants, write <name>.glb/.svg/stats.json
...
#include "engine/procgen/lanelab/level_import.h"
#include <fstream>
...
int main(int argc, char** argv) {
    if (argc < 3 || (std::strcmp(argv[1], "build") != 0 && std::strcmp(argv[1], "from-level") != 0 && std::strcmp(argv[1], "twin") != 0 && std::strcmp(argv[1], "blocks") != 0)) {
        std::fprintf(stderr, "usage: lanelab_tool build <graph.json> [--out <dir>]\n"
                             "       lanelab_tool from-level <level.json> --out <dir> [--diamonds N] [--no-freeway]   (writes <dir>/<name>_lanelab.json + terrain grid)\n"
    ...
    if (std::strcmp(argv[1], "from-level") == 0) {
        ...
        } catch (const std::exception& e) { std::fprintf(stderr, "lanelab from-level: %s\n", e.what()); return 1; }
        return 0;
    }
    std::unique_ptr<Result> rp;
    try { rp = build(RoadLabGraph::load(argv[2])); } catch (const std::exception& e) { std::fprintf(stderr, "lanelab: %s\n", e.what()); return 1; }
    Result& r = *rp;
    ...
    if (!debugLane.empty()) {   // the lane's footprint (raw rings), its spine and every deck triangle within its bbox, by owner
        ...
        if (!writeGlb(meshes, base + ".glb", &err)) std::fprintf(stderr, "lanelab: %s\n", err.c_str());
