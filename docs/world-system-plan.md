...
*not* a turtle/plant L-system — likely the same Lua-recipe substrate with a
different operation set. Interiors + exterior LOD + interior streaming are far
future. **Open decision:** split-grammar-as-Lua vs a dedicated node graph for
buildings — deferred until the plant/scatter recipes prove the substrate.
