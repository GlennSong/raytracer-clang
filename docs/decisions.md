...
**Third person on foot, and the player IS a person (follow-up).** Two seams the
generic camera + shared walker body made cheap. (1) The on-foot camera toggles
first ↔ third person on **V** (guarded so it stands down while any placed
`SceneCamera` owns V's viewport-cycle meaning); third person reuses
`FollowCameraController` with a new over-the-shoulder preset (short arm, a
right-shoulder `lateralOffset` that lives in the follow frame so it turns with
the heading, aim above head height). Mouse-look still steers the player heading
via the same fly controller in both modes — the shoulder rig only frames it,
fly pitch tilting the rig; the eye stays pinned in both modes so the gun script
and a seamless switch-back keep working. (2) In third person the player renders
as the **same articulated person the city walkers use** (`buildPersonMesh`) in
a reserved, vivid outfit (last table slot, never dealt to a walker), its walk
cycle driven by the capsule's real horizontal speed through the now-shared
`walkPoseIndex`/`walkPoseSwing`/`kWalk*` — so the player and the crowd stride
identically. The capsule stays the collider; only its visual changes (mesh
scaled to the capsule height so the feet meet the ground), and it is removed in
first person (no head in the camera). `CityPlayerBodySystem` (apps/citysim) is
decoupled from `PlayerSystem` through a `playerThirdPerson` Settings flag, and
runs after it so the pose it reads is already the tick's. The camera math,
shoulder preset, walk-cycle helper, and reserved outfit are headless-tested
(`test_follow_camera.cpp`, `test_city_render.cpp`); the two systems are
viewer/Jolt-gated and UNVERIFIED on device (register row below). Suite 677/677.

**Spectate camera — watch an NPC live its life (follow-up).** A third seam the
generic FollowCameraController makes cheap: `CitySpectateSystem` (apps/citysim)
lets you cycle the view through the living city's agents and chase one, so you
watch an NPC drive its commute or walk its route with its debug ring + vision
cone lit. **K** toggles spectate on/off; **[** / **]** cycle the followed agent
(prev / next), skipping released/player ghosts and wrapping. Turning it on forces
the debug widgets on (via a new `CityRenderSystem::setDebugWidgets`, stashing and
restoring the prior state rather than fighting the `j` toggle) so the followed
agent's ring/cone show; turning it off writes nothing and hands the view back to
PlayerSystem/CameraSystem (it only overrides `ctx.view.camera` WHILE spectating,
and is registered after them so its override wins the frame). The followed
agent's world pose comes from a new read accessor `CityRenderSystem::agentWorldPose`
— the real external body when the vehicle/walker bridges own it (same source the
debug widgets ring), else the sim ghost. The chase camera frames a driver from a
longer arm and a walker tighter; it is fed ZOOM ONLY (mirroring the third-person
branch) so the agent's heading owns the yaw and look input never double-applies.
It only reads poses + drives the camera, so it needs no Jolt and lives OUTSIDE
the physics block (it follows sim ghosts even in a physics-off build). Two new
`[` / `]` `KeyCode`s were added (GLFW + Qt mappings). The pure parts —
`nextSpectatable` (cycle/wrap/skip), `clampSpectateIndex` (stale-index), the
heading→degrees conversion, and the empty/one-agent edge cases — are headless
...
**Follow-up — the markers become lamps.** The retained `lights` markers now DRIVE
emissive lamps, faithful to the signal-lens pattern (one instance group per lamp
kind, transforms rebuilt each step from per-car state). A pure decision core
`carLampState(state, speed, prevSpeed, clockHours, turnDir)`
(`apps/citysim/car_lamps.h`) — no ECS, no Lua — decides which lamps are lit:
headlights before dawn / after dusk off the sim's own deterministic clock
(`CitySim::timeOfDay`), brake lights when the car holds (Waiting/Yielding) or
sheds speed hard while already slow, and the turn-signal SIDE (via `carTurnSide`
on the route bend, the same read as the FSM's `bendAhead`) while Turning.
`city_render.cpp` adds three groups mirroring the signal lenses — headlight
(white), brake (red), turn (amber) — and `syncCarLamps` pushes a small emissive
box (`lampMaterial`, FLAG none exactly like the signal lens) at each lit marker's
WORLD pose (car pose · local marker) every step; only lit lamps are pushed, so a
phase change just moves transforms between batches. The turn indicator blinks
~1.5 Hz off the sim clock (deterministic, so the bake is reproducible and
headless-testable). Cars on the C++ fallback fleet — or any Lua-free / headless
build, where there are no Lua markers — synthesize a default marker set from the
body size so they light up too. The predicate is covered exhaustively headless
(`test_car_lamps.cpp` — dusk/dawn boundaries, brake on hold + hard decel, turn
side, all-off daylight cruise, `carTurnSide` geometry) and the instanced bake
headless with null meshes (`test_city_render.cpp` — night headlights, brake on
...
| ~~City pedestrians render as boxes~~ | ~~`apps/citysim/city_render.cpp`, `apps/citysim/city_walkers.cpp`~~ | *Resolved (ADR-0063): walkers are the six-box articulated `buildPersonMesh` (head/torso/swinging limbs, 12 outfits), walk cycle driven by real body speed through the shared `walkPoseIndex`; the third-person player reuses the same body (ADR-0064). Owed only a device look-pass (bundled with the third-person row below).* | — |
...
| Fleet car lamps are UNVERIFIED on device | `apps/citysim/car_lamps.h`, `apps/citysim/city_render.cpp` (`syncCarLamps`/`carTurnDir`), `apps/citysim/city_meshes.cpp` (`lampMaterial`), `assets/scripts/vehicles.lua` (`vehicle.fleet[*].lights`), `engine/scripting/vehicle_body.cpp` (`CarBodyRecipe::lights`) (ADR-0065) | RESOLVED IN DESIGN, owed a viewer pass: the Lua fleet's `headlight_l/r` + `taillight_l/r` markers are now RETAINED per variant (was: parsed-and-dropped) and baked each step into three emissive instance groups mirroring the signal lenses — white headlights (on at dusk/dawn off the sim clock), red brake lights (holding at a signal / hard decel while slow), amber turn signals (route-bend side, ~1.5 Hz sim-clock blink) — driven by the pure `carLampState` predicate + `carTurnSide`. Fallback cars (C++ `fleetCarMesh`, scripting off, headless) synthesize a default marker set so they light up too. The decision core is headless-tested (`test_car_lamps.cpp`) and the instanced bake is exercised headless with null meshes (`test_city_render.cpp`: night headlights, brake-on-hold, turn-signal flash, externally-owned cars draw none); the GPU draw of the emissive quads needs a viewer and is unrun here. | Viewer pass: confirm the lamps sit on the drawn lamp geometry, read at street distance, and the emissive tuning glows through day/bloom; then (deferred) projected headlight cones and separate amber indicator lenses / lamps for the C++ fallback fleet |
...
| Road earthwork + retaining walls are device-UNVERIFIED | `engine/procgen/terrain.{h,cpp}` (DaylightBatter), `engine/procgen/city/road_net.cpp` (`roadNetConformRegions`/`buildRoadWalls`), `engine/procgen/city/structure_set.{h,cpp}`, `engine/level_loader.cpp` (ADR-0075 P1) | RESOLVED IN DESIGN, owed a CDLOD-viewer pass: roads now cut a real batter into the uphill slope and fill an embankment downhill, daylighting into natural ground (replacing the smoothstep feather); a steep cut that can't daylight within the ~12 m reach gets a concrete retaining/fill wall (StructureSet) with a static collider. Verified end-to-end HEADLESS through the real engine path (`roadNetConformRegions` + `terrainHeight`) on a hillside — cross-sections read right, walls trigger on steep cuts, terrain-index parity holds — but the CDLOD mesh + the wall entity's render/collider have never been seen on a Metal/Vulkan build here. Owed checks: the carved batter reads clean on the LOD terrain (no coarse-node poke-through at the steeper cut), the walls sit on the batter top and cap the step, the concrete material + collider read right, and the reach/batter constants look right on real mountain terrain (tune if a 12 m reach eats too much or leaves raw cliffs). | Load a metro on hilly CDLOD terrain in the viewer; tune reach/cutBatter/fillBatter + wall material; confirm the collider and the coarse-LOD carve |
| Block grading cascade is device-UNVERIFIED | `engine/procgen/city/block_grade.{h,cpp}`, `engine/level_loader.cpp` (lot pre-pass), `engine/procgen/terrain.{h,cpp}` (TerrainFlatten priority) (ADR-0075 P2) | RESOLVED IN DESIGN, owed a viewer pass: each block is graded down to its bounding streets (a plane fit to the road-carved boundary heights) before lots grow, and terraced into flat steps where the drop is too steep for one plane — so building pads flatten on ground that tilts with the street, not the raw noise. An overlap PRIORITY on TerrainFlatten keeps pads/roads (0) winning over the block grade (-1) where they overlap, so pads stay flat. Verified HEADLESS end to end (gradeBlocks + applyFlatten on a hillside: gentle blocks tilt as one plane, steep terrace; a pad stays flat over a tilted block) but the building-placement RESULT on the CDLOD terrain — do buildings sit right on graded/terraced blocks, do terrace steps read, does a downhill-frontage pad still ground cleanly — has never been seen. Owed checks: buildings sit flat + grounded on graded blocks, terrace steps look intentional (tune maxDrop), rim (outskirt) blocks aren't graded yet (only enclosed faces), and the block-grade flatten doesn't fight the road carve at the frontage. | Load metro_hills.json in the viewer; confirm buildings grade with their block; tune maxDrop / falloff; grade rim blocks too if the outskirts read raw |
...
| Water rendering wired; the Water SHADER is device-UNVERIFIED | `engine/procgen/city/water_mesh.{h,cpp}`, `engine/level_loader.cpp`, `src/level_scene.cpp`, `assets/levels/coast.json` | `buildWaterMesh` (a flat plane at seaLevel emitted only where the natural floor dips below it, depth + shore-distance baked into UV) existed but was wired into NEITHER loader — it only ran in scratch probes, so the water system had never been seen. Both loaders now read a `"water"` block ({seaLevel, region/lo/hi, cell, foamBand, color}), build the mesh over the level's natural `levelGround` floor, and add it — the viewer as a `Surface::Water` Renderable (low roughness + <1 opacity for the waves/foam/depth shader on windTime), the offline tracer as a glossy dielectric. `coast.json` is the showcase (a drowned coastline + snowy range). Verified OFFLINE (the bay renders, deep-blue with a mountain reflection); the Water surface SHADER (animated waves, depth-graded colour, shoreline foam, SSR) is Metal/Vulkan/WGSL and UNVERIFIED here. Owed: bridges (a road that `crossesWater` needs a bridge deck — ADR-0054 primitive), and the debug-panel water/land viz (task #117). | Viewer pass on each backend: confirm waves/foam/depth read + reflect; then wire bridges over `crossesWater` spans and the buildability land-class overlay |
| Terrain material (micro-relief) is Metal-only, device-UNVERIFIED | `renderer/renderer.h` (`Surface::TerrainGround`), `shaders/metal/surface_terrain.metal` (`surfTerrain` + `surfaceReliefTerrain` — both halves now live together), `engine/level_loader.cpp` | The terrain was PBR-LIT but had no material — flat vertex colour, uniform 0.93 roughness, no normal detail (buildings/roads get procedural surfaces; ground got none). `Surface::TerrainGround` now gives the ground a real surface like the road: a slope-scaled procedural NORMAL perturbation (steep rock tilts hard, flat sand/grass gentle) + roughness variation (rock rough, high+flat snow glossier) from world-planar value noise, plus fine albedo grain — the biome COLOUR stays in the vertex colour (Layer 1), this only adds relief. Set on the CDLOD + static terrain material in the viewer loader (not offline — the path tracer keeps flat vertex colour). Metal ONLY; Vulkan/WGSL have no terrain surface entry (unknown surfId 13 is a no-op there, so web/Vulkan terrain is just the vertex colour — graceful). UNVERIFIED: no Metal build here, mirrors the proven road block but unrun. | Device pass: tune the relief amp/roughness + octave freqs on real terrain; then per-biome detail (sand ripples vs rock vs snow sparkle, keyed off the vertex colour or a baked biome id) and Vulkan/WGSL parity |
...
| Ground truth is still TWO authorities; placement value-matches the mesher | `engine/procgen/terrain_lod.cpp` (leaf dilate `step*1.45` + road clamp), `engine/level_loader.cpp` (`plantingGround`) | The planting round (12d1a8f) fixed floating objects by making every placement path sample the carved field **with the mesher's leaf-level dilate** — a shared helper whose comment points at terrain_lod. That is a *value-matched coupling*, not a unification: the mesher and placement still compute ground independently, and a change to the mesher's dilate/clamp policy silently re-floats every tree and pole. The real fix is ONE ground-truth module (field + dilate + clamp + interpolation in one place) that the mesher, the colliders, and placement all query — or placement raycasting the generated leaf mesh itself. NOW GUARDED: the floorplan-conformance census (test_lot_conformance.cpp + the level_tests gate) replays the mesher's exact math over every building perimeter on every shipped level — a divergence between the authorities fails CI with coordinates instead of floating a building. | Next change to terrain_lod's dilate/clamp policy — the census will fail and point at it; build the single GroundTruth authority then |
| Plan polygons have no winding invariant; consumers self-defend | `engine/procgen/city/city_lots.cpp` (3× `ensureCCW`), `shape_grammar.cpp:2075` | The dead `area(p)<0` guards (area() is \|signedArea\|) inverted foundation skirts for ~half of buildings. Fixed by calling `ensureCCW` at the three consumption sites — but the architecture still lets a CW plan travel: every FUTURE consumer of `b.plan`/lot polys must remember to canonicalize or silently emit inside-out geometry. The invariant belongs at CREATION (canonicalize in the box-fallback/rowhouse/courtyard constructors, debug-assert CCW at consumers). | The next new plan consumer, or any new winding bug — canonicalize at the source then |
| Pad apron (2.2 m) is narrower than the deepest portico (~3.8 m) | `engine/level_loader.cpp:2374` (`kPadApron`), `shape_grammar.cpp:1694-1702` (portico depth) | The apron puts flat pad under skirts and bare entrance steps (~1.7 m reach) but porticos can reach ~3.8 m — on a steep falloff the outer portico columns can still hover. Steps/porticos are also still collider-less (buildings aren't enterable, so cosmetic today). PARTIALLY PAID (floorplan-conformance round): entrance steps/porticos now extend BELOW the storey base to the sampled entrance ground (BuildingParams::entranceDropBelow), so the stoop lands on real ground. Residual: terrain under a portico's outer 1.6 m is pad-falloff, not pad plane, and steps still have no collider. | When buildings become enterable (colliders then matter) |

| Road growth ignores vertical context: brink-hugging + 16 m cross-road deltas | `engine/procgen/city/metro.cpp` (growth), `road_net.cpp` (conform) | Measured on metro_v2_test's west rim (`ground?` probe): two roads 30 m apart horizontally at 60.7 m and 44.9 m — each conforms terrain to itself, leaving a cliff between; the rim road hugs the island's 60 m outer shelf. The band-skirt drape + the lot relief gate (rejRelief) now render these seams HONESTLY (retaining faces, hillside lots go green), but the generator still CREATES them: growth checks per-road max_slope only, never cross-road vertical proximity or lateral clearance to steep shelves. Proper fixes: a grade-separation constraint at growth time (reject/reroute candidates that land within X m horizontal and > Y m vertical of an existing road), and engineered stepped retaining walls where a cut face exceeds ~4 m. | The next city-generation round (Metropolis polish), or the first switchback-heavy hill town |
