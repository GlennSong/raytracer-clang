...
| City pedestrians render as boxes | `apps/citysim/city_render.cpp` (ADR-0059 Phase 6) | Pedestrians are `MeshBuilder::box` instances. Cars now come in several vertex-coloured body styles (sedan/hatchback/SUV/pickup) across multiple instance groups; signals reuse the `street_kit` head with a lit lens. Functional + instanced, but the ped (and the cars) are still primitive procedural meshes, not authored art. | Authored/instanced pedestrian + richer vehicle models (a content pass); Lua-authored vehicle bodies already exist (ADR-0058) and could feed the car groups |
| CitySim crosswalks are decals, not road paint | `apps/citysim/city_render.cpp` (ADR-0059) | Zebra crosswalks at junction mouths are baked white quads laid ~4 cm above the asphalt by the render bridge (opaque, so they cover the centreline where they cross). The road mesh's own crosswalk path (`buildRoadMesh` `crosswalks`, with a proper centreline inset) is only in the opt-in analytic mesher, not the default weld; the decal sidesteps that. | Generate crosswalks + a junction centreline inset in the default weld mesher (road-geometry plan), then drop the decal |
...
| AI cars can overlap at junctions | `apps/citysim/city_sim.cpp` (ADR-0059) | Car-following prevents same-lane overlap and signals gate signalled junctions, but cross traffic (esp. at unsignalized junctions, and turning movements inside a green phase) can still occupy the same space — cars visually intersect. Kinematic collider proxies don't resolve car-vs-car. Braking for all cross cars in a cone previously deadlocked traffic, so it was removed. | A deadlock-free junction reservation / right-of-way (single-owner per junction with deterministic priority), tested for both no-overlap and no-gridlock |
...
| Pedestrians don't locally avoid | `apps/citysim/city_sim.cpp` (ADR-0059) | Peds follow the sidewalk centreline; they don't steer around each other, the player, or signal poles (they can clip through). Ped colliders block the player but peds still path through each other kinematically. | Local avoidance (steer around near obstacles) for peds; and a knock-down-then-recover physics state when struck |
