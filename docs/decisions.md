...
| Terrain material (micro-relief) is Metal-only, device-UNVERIFIED | `renderer/renderer.h` (`Surface::TerrainGround`), `shaders/metal/common.metal` (`surfTerrain`), `shaders/metal/lighting.metal` (surfId 13), `engine/level_loader.cpp` | The terrain was PBR-LIT but had no material — flat vertex colour, uniform 0.93 roughness, no normal detail (buildings/roads get procedural surfaces; ground got none). `Surface::TerrainGround` now gives the ground a real surface like the road: a slope-scaled procedural NORMAL perturbation (steep rock tilts hard, flat sand/grass gentle) + roughness variation (rock rough, high+flat snow glossier) from world-planar value noise, plus fine albedo grain — the biome COLOUR stays in the vertex colour (Layer 1), this only adds relief. Set on the CDLOD + static terrain material in the viewer loader (not offline — the path tracer keeps flat vertex colour). Metal ONLY; Vulkan/WGSL have no terrain surface entry (unknown surfId 13 is a no-op there, so web/Vulkan terrain is just the vertex colour — graceful). UNVERIFIED: no Metal build here, mirrors the proven road block but unrun. | Device pass: tune the relief amp/roughness + octave freqs on real terrain; then per-biome detail (sand ripples vs rock vs snow sparkle, keyed off the vertex colour or a baked biome id) and Vulkan/WGSL parity |
| Water surface has no ripples/animation; foam is static, Metal-only | `shaders/metal/common.metal` (`surfWater`), `renderer/{metal,vulkan,webgpu}` | `surfWater` grades colour by baked depth and paints a STATIC shoreline foam band (noise edge on the baked shore-distance) + gentle static colour mottle. There are NO ripple normals (the plane shades flat, so it doesn't glint like rippled water), NO wave animation, and NO "ocean" model (no Gerstner/FFT/shear was ever built). It is Metal-ONLY — Vulkan/WebGPU `mesh.frag`/`.wgsl` have no water surface entry, so water there is just a flat translucent colour. Wind time isn't consumed. | Static (time-independent, no wind) ripple NORMALS perturbing N before lighting/SSR + a stronger foam line; then port `surfWater` to Vulkan/WGSL for parity; wind-driven waves later |
