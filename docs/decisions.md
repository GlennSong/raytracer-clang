...
| City pedestrians render as boxes | `apps/citysim/city_render.cpp` (ADR-0059 Phase 6) | Pedestrians are `MeshBuilder::box` instances. Cars use a simple vertex-coloured car silhouette (body + cabin + wheels); signals reuse the `street_kit` head with a lit lens. Functional + instanced, but the ped (and the car) are not authored models. | Authored/instanced pedestrian + richer vehicle models (a content pass); Lua-authored vehicle bodies already exist (ADR-0058) and could feed the car group |
...
| AI city cars have no physics collision | `apps/citysim/city_render.cpp` (ADR-0059 Phase 6) | AI cars are kinematic InstanceGroup transforms — no Jolt body, so the physics gun/player can't bump them and they don't collide with world geometry. Only the player's own car runs Jolt (`VehicleSystem`). | The hybrid handoff (below): spawn a Jolt vehicle for AI cars near the camera, kinematic far away |
