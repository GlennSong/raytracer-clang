...
| City cars/pedestrians render as boxes | `apps/citysim/city_render.cpp` (ADR-0059 Phase 6) | Cars and pedestrians are `MeshBuilder::box` instances. Functional and instanced, but not authored models. (Signals now reuse the city's `street_kit` traffic-signal assembly with a lit emissive lens per phase — see below.) | Authored/instanced car + pedestrian models (a content pass); Lua-authored vehicle bodies already exist (ADR-0058) and could feed the car group |
