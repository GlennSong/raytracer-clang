...
| Pedestrian avoidance is separation-only | `apps/citysim/city_sim.cpp` (ADR-0059) | Peds now push apart (a few relaxation iterations each step) so their bodies don't overlap each other or the player — they step around rather than through. Still missing: avoidance of signal POLES (render-side, not sim agents), longitudinal following (peds queue close at a shared destination), and a knock-down-then-recover state when struck. | Ped-vs-pole avoidance (feed pole positions into the sim), simple ped following, and a ragdoll/knock-down physics state |
