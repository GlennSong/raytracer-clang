...
| Beacon flash `BeaconBlink` + `beaconBlinkGate` | `engine/components.h`, `vehicle_lamps.h`, `systems/day_night_system.cpp` (`applyBeaconBlink`), `PartId::{Beacon,BeaconGlow,BeaconHaze}` | tall-roof aviation lamps (loader tags the 24 m beacon chunks) | dusk ramp × a hashed-phase flash cycle written per frame, divided by the night exposure adaptation, and scaling a translucent halo's opacity too; the pattern for any other flashing or glowing night light |
