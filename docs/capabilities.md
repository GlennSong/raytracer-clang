...
| Beacon flash `BeaconBlink` + `beaconBlinkGate` | `engine/components.h`, `vehicle_lamps.h`, `systems/day_night_system.cpp` (`applyBeaconBlink`), `PartId::Beacon` | tall-roof aviation lamps (loader tags the 24 m beacon chunks) | dusk ramp × a hashed-phase flash cycle written per frame; the pattern for any other flashing night light |
