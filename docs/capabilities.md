...
| Light LOD block `CitySimConfig::light{SpriteIn,SphereOut,Radius,Range,Count}` | `engine/components.h`; level JSON `citysim` | beacon tiers (sprite fade-in, sphere fade-out, point-light radius/range/count) | level-authored beside `detailDistance`; any small light's tiers should read these |
