...
| Terrain horizon shadows (mountains) | ❌ | ✅ | 2026-08-26: `terrainShadow` in mesh.frag — an R8 horizon map (set 0 binding 4; `setTerrainHorizon`) of sin(horizon elevation) toward light 0, marched on the CPU from a 256² raster of the analytic terrain (`engine/procgen/terrain_horizon.*`, job-pool rows) whenever the light moves > 2°; multiplies the cascade visibility. The camera-side disc occlusion is backend-agnostic (CPU). Metal/WebGPU: the `setTerrainHorizon` seam is a no-op — bind the texture and port the 12-line lookup. |
...
| Volumetric clouds (slab + shell) | ✅ | 🟡 | 2026-08-23: VULKAN WIRED — half-res march + depth-guided bilateral composite onto HDR before post, the shared cloud_noise.h Perlin-Worley bake uploaded as two 3D textures, CloudUniforms in a per-frame UBO ring; clouds.frag rewritten to the CURRENT clouds.metal (weather map, stratified 2x2 dither + IGN, two step sizes, zero-run stretch + entry refinement, analytic in-scatter, aerial fade). Device-run on the metro: deck renders over the city, 2D overlay retired while active. Slice-banding on far undersides + top stipple need a Metal same-pose A/B; the scattering-sky sun-transmittance tint is skipped (no scattering sky on Vulkan yet). Prior note (both backends unwired) is obsolete — Metal wired 2026-07-25. |
