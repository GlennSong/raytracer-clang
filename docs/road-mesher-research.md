...
1. Whether the `drawTerrain` pipeline can dispatch surface id 11 (`RoadMarkings`) — surface dispatch is in the fragment shader (`applySurface` in `surfaces.metal`), so it probably can, but the terrain vertex shader may not forward `u/v`.
2. `terrain_lod.h` says "only the height differs on a regular grid" — the shader (`terrainVertexMain` in `lighting_entry.metal`) may lerp only Y. **A road curves in XZ**, so an odd ring on a bend must slide toward the coarse chord laterally too. You need a full-`Vec3` morph. Check `mix(pos, morphTarget, k)` actually does that before assuming `drawTerrain` is a drop-in.
...
6. **UV semantics live in four places.** `surface_road.metal` says the RoadMarkings logic is mirrored in WGSL, Vulkan and `scene.cpp`. **I verified only the Metal one.** Stages 1–3 need no shader change (that is the design's biggest gift), but Stage 5's lane-local `mu` would touch all four.
