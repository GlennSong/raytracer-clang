...
### 3.5 Curved internodes + rotation-minimizing frame
Smooth the per-branch node polyline (Catmull-Rom) and carry a **parallel-
transport (rotation-minimizing) frame** instead of the accumulated `Mat4`
orientation, which drifts in roll. Gives gentle natural curvature and — crucial
for §4 — twist-free bark UVs. (ABoP renders straight internodes; this is our
real-time upgrade.)
